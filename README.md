...
- 정현수: 프로젝트를 진행 중간중간에 모델이랑 url를 수정하면서 기획의 중요성을 깨닫게 되었습니다. 또한 vue 가장 많이 본 에러가 undefined였는데 시간적으로 동작처리 순서를 공부할 필요성을 느끼게 되었습니다. 삼일 전만해도 에러가 많이 나서 시간안에 고칠 수 있을 지 걱정 되었는데, 에러를 해결하고 그 안에 결과물을 만들어 내어 뿌듯합니다. 아쉬운 점은 vue에서 몇가지 재사용될 수 있는 컴포넌트들을 분리 못시킨 점이 아쉽고, 계속 undefined 에러가 뜨길래 되도록이면 props를 안쓰고 store에 저장해서 쓰는 방식을 택했는데 기회가 된다면 props로 전달해서 받는 방식으로도 도전해 보고 싶습니다. 이번 프로젝트를 진행하면서 에러 문구를 보고 해석하는 방법, 그리고 인터넷 검색하는 방법을 배울 수 있었습니다. 그리고 프로젝트를 하면서 의견차이를 보일 때도 있었는데 그러면서 소통하는 방법을 배우게 된 것 같습니다.
...
## 예상 질문
### 질문 1: 금융상품 추천 알고리즘은 어떻게 구현되었나요?
답변:
금융상품 추천 알고리즘은 사용자 데이터와 금융상품 데이터를 기반으로 작동합니다. 주요 단계는 다음과 같습니다:

데이터 수집 및 전처리:

사용자 프로필, 거래 내역, 관심사 등을 수집하여 전처리합니다.
금융상품의 특징 (예: 금리, 리스크, 기간 등)도 수집하고 전처리합니다.
사용자 분류:

사용자의 금융 성향을 분석하여 유사한 사용자 그룹으로 분류합니다.
군집화 알고리즘(K-means, DBSCAN 등)을 사용하여 사용자 그룹을 형성합니다.
추천 알고리즘:

협업 필터링(Collaborative Filtering): 비슷한 성향의 사용자들이 선호하는 금융상품을 추천합니다.
콘텐츠 기반 필터링(Content-Based Filtering): 사용자가 이전에 선호했던 금융상품과 유사한 제품을 추천합니다.
하이브리드 방식: 협업 필터링과 콘텐츠 기반 필터링을 결합하여 보다 정교한 추천을 제공합니다.
평가 및 개선:

추천의 정확도를 평가하기 위해 정밀도(Precision), 재현율(Recall) 등의 지표를 사용합니다.
사용자 피드백을 바탕으로 알고리즘을 지속적으로 개선합니다.

### 질문 2: 금융상품 추천 탭의 성능 최적화는 어떻게 이루어졌나요?
답변:
금융상품 추천 탭의 성능 최적화를 위해 다음과 같은 접근 방식을 사용했습니다:

데이터베이스 최적화:

인덱스를 적절히 사용하여 쿼리 성능을 향상시켰습니다.
데이터베이스 정규화를 통해 중복 데이터를 최소화하고, 필요한 경우 역정규화(Denormalization)를 통해 읽기 성능을 개선했습니다.
캐싱(Caching):

Redis와 같은 인메모리 데이터베이스를 사용하여 빈번히 조회되는 데이터를 캐싱했습니다.
사용자별 추천 결과를 캐싱하여 반복적인 연산을 줄였습니다.
비동기 처리 및 큐 사용:

추천 알고리즘의 연산이 복잡하고 시간이 오래 걸리는 경우 비동기 처리(Async)와 작업 큐(Job Queue)를 사용하여 웹사이트의 응답 시간을 단축했습니다.
Celery와 같은 작업 큐를 사용하여 백그라운드에서 무거운 작업을 처리했습니다.
프론트엔드 최적화:

Lazy loading을 사용하여 필요한 경우에만 데이터를 로드하도록 했습니다.
클라이언트 측에서 데이터 페이징(Pagination)을 구현하여 한 번에 많은 데이터를 로드하지 않도록 했습니다.
웹팩(Webpack)과 같은 모듈 번들러를 사용하여 자바스크립트 파일을 최적화했습니다.
이러한 최적화 기법들을 통해 금융상품 추천 탭의 성능을 높이고 사용자 경험을 개선했습니다.
